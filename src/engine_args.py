...
DEFAULT_ARGS = {
    ...
    "disable_log_requests": os.getenv("DISABLE_LOG_REQUESTS", "False").lower()
    == "true",
    ...
    "max_parallel_loading_workers": int(os.getenv("MAX_PARALLEL_LOADING_WORKERS", 0))
    or None,
    "block_size": int(os.getenv("BLOCK_SIZE", 16)),
    "enable_prefix_caching": os.getenv("ENABLE_PREFIX_CACHING", "False").lower()
    == "true",
    "disable_sliding_window": os.getenv("DISABLE_SLIDING_WINDOW", "False").lower()
    == "true",
    "use_v2_block_manager": os.getenv("USE_V2_BLOCK_MANAGER", "False").lower()
    == "true",
    ...
    # "max_context_len_to_capture": int(os.getenv("MAX_CONTEXT_LEN_TO_CAPTURE", 0))
    # or None,
    # "max_seq_len_to_capture": int(os.getenv("MAX_SEQ_LEN_TO_CAPTURE", 8192)),
    "disable_custom_all_reduce": os.getenv("DISABLE_CUSTOM_ALL_REDUCE", "False").lower()
    == "true",
    ...
    "enable_prompt_adapter": os.getenv("ENABLE_PROMPT_ADAPTER", "False").lower()
    == "true",
    ...
    "long_lora_scaling_factors": tuple(
        map(float, os.getenv("LONG_LORA_SCALING_FACTORS", "").split(","))
    )
    if os.getenv("LONG_LORA_SCALING_FACTORS")
    else None,
    "lora_dtype": os.getenv("LORA_DTYPE", "auto"),
    "max_cpu_loras": int(os.getenv("MAX_CPU_LORAS", 0)) or None,
    "device": os.getenv("DEVICE", "auto"),
    "ray_workers_use_nsight": os.getenv("RAY_WORKERS_USE_NSIGHT", "False").lower()
    == "true",
    ...
    "speculative_draft_tensor_parallel_size": int(
        os.getenv("SPECULATIVE_DRAFT_TENSOR_PARALLEL_SIZE", 0)
    )
    or None,
    "enable_expert_parallel": bool(
        os.getenv("ENABLE_EXPERT_PARALLEL", "False").lower() == "true"
    ),
    ...
    "speculative_disable_by_batch_size": int(
        os.getenv("SPECULATIVE_DISABLE_BY_BATCH_SIZE", 0)
    )
    or None,
    ...
    "spec_decoding_acceptance_method": os.getenv(
        "SPEC_DECODING_ACCEPTANCE_METHOD", "rejection_sampler"
    ),
    "typical_acceptance_sampler_posterior_threshold": float(
        os.getenv("TYPICAL_ACCEPTANCE_SAMPLER_POSTERIOR_THRESHOLD", 0)
    )
    or None,
    "typical_acceptance_sampler_posterior_alpha": float(
        os.getenv("TYPICAL_ACCEPTANCE_SAMPLER_POSTERIOR_ALPHA", 0)
    )
    or None,
    "qlora_adapter_name_or_path": os.getenv("QLORA_ADAPTER_NAME_OR_PATH", None),
    "disable_logprobs_during_spec_decoding": os.getenv(
...
def match_vllm_args(args):
    ...
    matched_args = {
        k: v
        for k, v in renamed_args.items()
        if k in AsyncEngineArgs.__dataclass_fields__
    }
